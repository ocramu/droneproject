...
def automated_sequence(duration_seconds):
    ...
    try:
        ...
        # 2. Takeoff (Drone apps usually require holding this button to confirm)
        print("[AUTO] Pressing and Holding Large Takeoff...")
        mouse.position = BTN_LARGE_TAKEOFF
        mouse.press(Button.left)
        if not check_sleep(2.5): # Hold for 2.5 seconds
...
def process_key_press(key):
    ...
    try:
        ...
        # Ignore all manual input (except space/esc) if automation is running
        if auto_sequence_running:
            return
...
def process_key_release(key):
    try:
        ...
        # Only process releases if auto sequence isn't overriding the mouse
        if not auto_sequence_running:
...
if __name__ == "__main__":
    ...
    listener = keyboard.Listener(on_press=process_key_press, on_release=process_key_release)
    listener.start()

    # 3. Trigger the automated sequence in a background thread so the listener stays active
    auto_thread = threading.Thread(target=automated_sequence, args=(dist_input,), daemon=True)
    auto_thread.start()

    # Keep the main thread alive watching the listener
    listener.join()
